...
class WorkspaceDocumentationTests(unittest.TestCase):
    ...
    def test_agent_instructions_preserve_durable_workspace_safety(self):
        ...
        self.assertIn("selected `workspace refresh` is the only operation that may use an outbound network transport", normalized)
        self.assertIn("source viewer may use numeric loopback http only", normalized)
